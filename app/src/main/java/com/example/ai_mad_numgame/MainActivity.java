...
public class MainActivity extends AppCompatActivity {
    ...
    public void newMatch() {  //A game is composed of three matches
        ...
        if (matchCounter == 3) {    // if three matches are completed updates the performance in sharedpreferences
    ...
    public String getInterpretation(int[][] dataFrame, double slope) {
        ...
        if (slope > 0 && slope <= 0.5) {
            return "You are a beginner, need improvement!";
        } else if (slope > 0.5 && slope <= 0.75) {
            return "You are a good learner, keep it up!";
        } else if (slope > 0.75 && slope <= 1) {
            return "You are an excellent learner!";
        }
